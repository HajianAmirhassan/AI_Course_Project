...
agent1_info = {
    'enabled': True,
    'language': 'python', 
    'path': os.path.join(os.path.dirname(__file__), 'my_agent.py') 
}
...
while run:
    ...
    if round_over==False:
        if F1.alive==False:
            ...
            if score[0] + score[1] >= MAX_ROUNDS:
                print(f"Game Over! Final Score - F1: {score[0]}, F2: {score[1]}")
                if score[0] > score[1]:
                    print("F1 wins the game!")
                elif score[1] > score[0]:
                    print("F2 wins the game!")
                else:
                    print("Draw!")
                run = False
        elif F2.alive==False:
